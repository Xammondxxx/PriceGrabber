...
namespace PriceGrabber.GoogleVisionApi
{
    public class Data
    {
        ...
        {
            public static RequestJson.RootObject GetNewRequest(byte[] imageBytes)
            {
                return new RootObject()
                {
                    requests = new List<Request>()
                  {
                      new Request()
                      {
                           image = new Image() { content  =  System.Convert.ToBase64String(imageBytes)//"https://pbs.twimg.com/profile_images/2311298181/80d154u80lijwyufmcb6_400x400.png"
                           },
            ...
            {
                public string type { get; set; } = "TEXT_DETECTION";
